...
const SideBar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 z-50 h-screen w-64 bg-[#D9D9D9] py-6 px-6 flex flex-col justify-between transition-transform duration-300 ease-in-out sidebar-content ${
      ...
      >
        ...
        <div>
          ...
          <nav className="flex flex-col gap-10 mt-10">
            <Link to="/" className="flex items-center gap-4 text-xl text-[#7A6B3F] font-medium hover:opacity-80" onClick={onClose}>
              <img src={HomeIcon} alt="Home" className="w-7 h-7" />
              Home
            </Link>
            {isAdmin && (
              <Link to="/admin/dashboard" className="flex items-center gap-4 text-xl text-[#7A6B3F] font-medium hover:opacity-80" onClick={onClose}>
                <img src={Dashboard} alt="Dashboard" className="w-7 h-7" />
                Dashboard
              </Link>
            )}
            {isAdmin && (
              <Link to="/admin/upload" className="flex items-center gap-4 text-xl text-[#7A6B3F] font-medium hover:opacity-80" onClick={onClose}>
                <img src={Upload} alt="Upload" className="w-7 h-7" />
                Upload
              </Link>
            )}
            <Link to="/favourites" className="flex items-center gap-4 text-xl text-[#7A6B3F] font-medium hover:opacity-80" onClick={onClose}>
              <img src={Heart} alt="Favorites" className="w-7 h-9 " />
              Favorits
            </Link>
            <button onClick={logout} className="flex cursor-pointer focus:outline-none">
             <Link to={"/login"} className="flex items-center gap-4 text-xl text-[#7A6B3F] font-medium hover:opacity-80" onClick={onClose}>
